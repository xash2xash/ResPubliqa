...
*Setup instructions for a gitian build of ResPubliqa Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the ResPubliqa
...
to the ResPubliqa GitHub release page.
...
- [Building ResPubliqa Core](#building-respubliqa-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/respubliqa-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/respubliqa-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/respubliqa-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/respubliqa-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/respubliqa-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/respubliqa-win-${VERSION%\.*}-build.assert
```
...
You can also mail the files to Fuzzbawls (fuzzbawls@respubliqa.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/respubliqa-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/respubliqa-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/respubliqa-osx-*-build.assert
```
