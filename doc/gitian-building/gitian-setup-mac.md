...
The build script we'll be using is contained in the ResPubliqa github repository ([contrib/gitian-build.py](https://github.com/respubliqa-altc/respubliqa/blob/master/contrib/gitian-build.py)). Since this is a completely fresh environment, we haven't yet cloned the ResPubliqa repository and will need to fetch this script with the following commands:

```bash
curl -L -O https://raw.githubusercontent.com/respubliqa-altc/ResPubliqa/master/contrib/gitian-build.py
