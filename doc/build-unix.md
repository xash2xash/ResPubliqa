UNIX BUILD NOTES
====================
Some notes on how to build ResPubliqa in Unix.

Note
---------------------
Always use absolute paths to configure and compile ResPubliqa and the dependencies,
...
This will build respubliqa-qt as well, if the dependencies are met.
...
memory available when compiling ResPubliqa. On systems with less, gcc can be
...
To build ResPubliqa without wallet, see [*Disable-wallet mode*](/doc/build-unix.md#disable-wallet-mode)
...
If you want to build respubliqa-qt, make sure that the required packages for Qt development
...
Once these are installed, they will be found by configure and a respubliqa-qt executable will be
built by default.
...
Notes
-----
The release is built with GCC and then "strip respubliqad" to strip the debug
...
Security
--------
To help make your ResPubliqa installation more secure by making certain attacks impossible to
...
* _Non-executable Stack_: If the stack is executable then trivial stack-based buffer overflow exploits are possible if
    vulnerable buffers are found. By default, ResPubliqa should be built with a non-executable stack
...
When the intention is to run only a P2P node without a wallet, ResPubliqa may be compiled in
