...
#include "qt/respubliqa/send.h"
#include "qt/respubliqa/forms/ui_send.h"
#include "qt/respubliqa/addnewcontactdialog.h"
#include "qt/respubliqa/qtutils.h"
#include "qt/respubliqa/sendchangeaddressdialog.h"
#include "qt/respubliqa/optionbutton.h"
#include "qt/respubliqa/sendconfirmdialog.h"
#include "qt/respubliqa/myaddressrow.h"
#include "qt/respubliqa/guitransactionsutils.h"
...
SendWidget::SendWidget(ResPubliqaGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    ...
    btnContacts(new QPushButton())
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("You can transfer public coins (RSQ)"));
    setCssProperty(ui->labelSubtitle1, "text-subtitle");

    /* Address */
    ui->labelSubtitleAddress->setText(tr("ResPubliqa address or contact label"));
    ...
    ui->labelAmountSend->setText("0.00 RSQ");
...
void SendWidget::onSendClicked(){
    ...
    // will call relock
    if(!GUIUtil::requestUnlock(walletModel, sendPiv ? AskPassphraseDialog::Context::Send_RSQ : AskPassphraseDialog::Context::Send_zRSQ, true)){
...
bool SendWidget::sendZpiv(QList<SendCoinsRecipient> recipients){
    ...
    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        emit message(tr("Spend Zerocoin"), tr("zRSQ is currently undergoing maintenance."), CClientUIInterface::MSG_ERROR);
        return false;
    }
    ...
    // use mints from zRSQ selector if applicable
    ...
    if (walletModel->sendZpiv(
        ...
        inform(tr("zRSQ transaction sent!"));
        ZPivControlDialog::setSelectedMints.clear();
        clearAll();
        return true;
    } else {
        QString body;
        if (receipt.GetStatus() == ZRSQ_SPEND_V1_SEC_LEVEL) {
            body = tr("Version 1 zRSQ require a security level of 100 to successfully spend.");
        } else {
            int nNeededSpends = receipt.GetNeededSpends(); // Number of spends we would need for this transaction
            const int nMaxSpends = Params().Zerocoin_MaxSpendsPerTransaction(); // Maximum possible spends for one zRSQ transaction
        ...
        emit message("zRSQ transaction failed", body, CClientUIInterface::MSG_ERROR);
        return false;
    }
}
...
void SendWidget::onCoinControlClicked(){
    if(isRSQ){
        ...
        if (walletModel->getZerocoinBalance() > 0) {
            ...
            inform(tr("You don't have any zRSQ in your balance to select."));
        }
    }
}
...
void SendWidget::onRSQSelected(bool _isRSQ){
    isRSQ = _isRSQ;
    setCssProperty(coinIcon, _isRSQ ? "coin-icon-piv" : "coin-icon-piv");
    refreshView();
