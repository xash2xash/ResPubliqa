...
#include "qt/respubliqa/privacywidget.h"
#include "qt/respubliqa/forms/ui_privacywidget.h"
#include "qt/respubliqa/qtutils.h"
#include "guiutil.h"
#include "qt/respubliqa/denomgenerationdialog.h"
#include "qt/respubliqa/txviewholder.h"
...
PrivacyWidget::PrivacyWidget(ResPubliqaGUI* parent) :
    PWidget(parent),
    ui(new Ui::PrivacyWidget)
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("Minting zRSQ anonymizes your RSQ by removing any\ntransaction history, making transactions untraceable "));
    setCssSubtitleScreen(ui->labelSubtitle1);

    ui->labelSubtitle2->setText(tr("Mint new zRSQ or convert back to RSQ"));
    ...
    ui->lineEditAmount->setPlaceholderText("0.00 RSQ ");
    ...
    ui->labelValueDenom1->setText("0x1 = 0 zRSQ");
    ...
    ui->labelValueDenom5->setText("0x5 = 0 zRSQ");
    ...
    ui->labelValueDenom10->setText("0x10 = 0 zRSQ");
    ...
    ui->labelValueDenom50->setText("0x50 = 0 zRSQ");
    ...
    ui->labelValueDenom100->setText("0x100 = 0 zRSQ");
    ...
    ui->labelValueDenom500->setText("0x500 = 0 zRSQ");
    ...
    ui->labelValueDenom1000->setText("0x1000 = 0 zRSQ");
    ...
    ui->labelValueDenom5000->setText("0x5000 = 0 zRSQ");
    setCssProperty(ui->labelValueDenom5000, "text-body2");

    ui->layoutDenom->setVisible(false);

    // List
    ui->labelListHistory->setText(tr("Last zRSQ Movements"));
    ...
    // Only Convert to RSQ enabled.
    ui->containerViewPrivacyChecks->setVisible(false);
    onMintSelected(false);

    ui->btnTotalzRSQ->setTitleClassAndText("btn-title-grey", "Total 0 zRSQ");
    ui->btnTotalzRSQ->setSubTitleClassAndText("text-subtitle", "Show denominations of zRSQ owned.");
    ui->btnTotalzRSQ->setRightIconClass("ic-arrow");

    ui->btnCoinControl->setTitleClassAndText("btn-title-grey", "Coin Control");
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", "Select RSQ outputs to mint into zRSQ.");
    ...
    ui->btnResetZerocoin->setTitleClassAndText("btn-title-grey", "Reset Spent zRSQ");
    ui->btnResetZerocoin->setSubTitleClassAndText("text-subtitle", "Reset zerocoin database.");

    connect(ui->btnTotalzRSQ, SIGNAL(clicked()), this, SLOT(onTotalZpivClicked()));
...
void PrivacyWidget::onMintSelected(bool isMint){
    QString btnText;
    if(isMint){
        btnText = tr("Mint zRSQ");
        ui->btnCoinControl->setVisible(true);
        ui->labelSubtitleAmount->setText(tr("Enter amount of RSQ to mint into zRSQ"));
    }else{
        btnText = tr("Convert back to RSQ");
        ui->btnCoinControl->setVisible(false);
        ui->labelSubtitleAmount->setText(tr("Enter amount of zRSQ to convert back into RSQ"));
    }
...
void PrivacyWidget::onTotalZpivClicked(){
    ...
    if(!isVisible){
        ...
        ui->btnTotalzRSQ->setRightIconClass("btn-dropdown", true);
    }else{
        ui->layoutDenom->setVisible(false);
        ui->btnTotalzRSQ->setRightIconClass("ic-arrow", true);
    }
}
...
void PrivacyWidget::onSendClicked(){
    ...
    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        warn(tr("Zerocoin"), tr("zRSQ is currently undergoing maintenance"));
        return;
    }
    ...
    if(!GUIUtil::requestUnlock(walletModel, AskPassphraseDialog::Context::Mint_zRSQ, true)){
        inform(tr("You need to unlock the wallet to be able to %1 zRSQ").arg(isConvert ? tr("convert") : tr("mint")));
        return;
    }
...
void PrivacyWidget::mint(CAmount value){
    ...
    if(!walletModel->mintCoins(value, CoinControlDialog::coinControl, strError)){
        ...
        // Mint succeed
        inform(tr("zRSQ minted successfully"));
        // clear
...
void PrivacyWidget::spend(CAmount value){
    ...
    if(!walletModel->convertBackZpiv(
        ...
        // Spend succeed
        inform(tr("zRSQ converted back to RSQ"));
        // clear
        ui->lineEditAmount->clear();
    }
}
...
void PrivacyWidget::onCoinControlClicked(){
    if(ui->pushRight->isChecked()) {
        if (walletModel->getBalance() > 0) {
            ...
            inform(tr("You don't have any RSQ to select."));
        }
    }
}
...
void PrivacyWidget::updateDenomsSupply(){
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zRSQ </b>";
    ...
    ui->btnTotalzRSQ->setTitleText(tr("Total %1").arg(GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true)));
}
