...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset){
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::RSQ);
    ...
    if (settings.contains("nAnonymizeResPubliqaAmount") || reset)
        SoftSetArg("-anonymizerespubliqaamount", settings.value("nAnonymizeResPubliqaAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in respubliqa.cpp during shotdown to also remove the window positions
