// Copyright (c) 2017-2019 The ResPubliqa developers
...
CzRSQWallet::CzRSQWallet(std::string strWalletFile)
{
    ...
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZRSQSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZRSQSeed_deprecated()) {
                    LogPrintf("%s: Updated zRSQ seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
...
bool CzRSQWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZRSQCount(nCountLastUsed);
    else if (!walletdb.ReadZRSQCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzRSQWallet::Lock()
{
    seedMaster = 0;
}

void CzRSQWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzRSQWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZRSQ(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzRSQWallet::LoadMintPoolFromDB()
{
...
void CzRSQWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzRSQWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzRSQWallet::SyncWithChain(bool fGenerateMintPool)
{
...
bool CzRSQWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZRSQ(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
...
void CzRSQWallet::SeedToZRSQ(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzRSQWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzRSQWallet::GenerateDeterministicZRSQ(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzRSQWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzRSQWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzRSQWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
