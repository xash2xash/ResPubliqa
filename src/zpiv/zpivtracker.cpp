// Copyright (c) 2018-2020 The ResPubliqa developers
...
CzRSQTracker::CzRSQTracker(std::string strWalletFile)
{
...
CzRSQTracker::~CzRSQTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzRSQTracker::Init()
{
...
bool CzRSQTracker::Archive(CMintMeta& meta)
{
...
bool CzRSQTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzRSQTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzRSQTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzRSQTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzRSQTracker::GetSerialHashes()
{
...
CAmount CzRSQTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzRSQTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzRSQTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzRSQTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzRSQTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzRSQTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzRSQTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzRSQTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzRSQTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzRSQTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzRSQTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzRSQWallet* zRSQWallet)
{
    bool iszRSQWalletInitialized = (NULL != zRSQWallet);
    CMintMeta meta;
    ...
    if (! iszRSQWalletInitialized)
        zRSQWallet = new CzRSQWallet(strWalletFile);
    meta.isSeedCorrect = zRSQWallet->CheckSeed(dMint);
    if (! iszRSQWalletInitialized)
        delete zRSQWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzRSQTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzRSQTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzRSQTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzRSQTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzRSQTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzRSQTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzRSQWallet* zRSQWallet = new CzRSQWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zRSQWallet);
        }
        delete zRSQWallet;
