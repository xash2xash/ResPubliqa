...
public:
    ...
    bool WriteZRSQSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadZRSQSeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadZRSQSeed_deprecated(uint256& seed);
    bool EraseZRSQSeed();
    bool EraseZRSQSeed_deprecated();

    bool WriteZRSQCount(const uint32_t& nCount);
    bool ReadZRSQCount(uint32_t& nCount);
