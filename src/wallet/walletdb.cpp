...
void ThreadFlushWalletDB(const std::string& strFile)
{
    ...
    RenameThread("respubliqa-wallet");
...
bool CWalletDB::WriteZRSQSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed)
{
...
bool CWalletDB::EraseZRSQSeed()
{
    uint256 hash;
    if(!ReadCurrentSeedHash(hash)){
        return error("Failed to read a current seed hash");
    }
    if(!WriteZRSQSeed(hash, ToByteVector(base_uint<256>(0) << 256))) {
...
bool CWalletDB::EraseZRSQSeed_deprecated()
{
    return Erase(std::string("dzs"));
}

bool CWalletDB::ReadZRSQSeed(const uint256& hashSeed, std::vector<unsigned char>& seed)
{
    return Read(std::make_pair(std::string("dzs"), hashSeed), seed);
}

bool CWalletDB::ReadZRSQSeed_deprecated(uint256& seed)
{
    return Read(std::string("dzs"), seed);
}

bool CWalletDB::WriteZRSQCount(const uint32_t& nCount)
{
    return Write(std::string("dzc"), nCount);
}

bool CWalletDB::ReadZRSQCount(uint32_t& nCount)
{
