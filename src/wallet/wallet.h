...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 RSQ at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zRSQ send
enum ZerocoinSpendStatus {
    ZRSQ_SPEND_OKAY = 0,                            // No error
    ZRSQ_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZRSQ_WALLET_LOCKED = 2,                         // Wallet was locked
    ZRSQ_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZRSQ_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZRSQ_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZRSQ_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZRSQ_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZRSQ_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZRSQ_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZRSQ_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZRSQ_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZRSQ_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZRSQ_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZRSQ_SPENT_USED_ZRSQ = 14,                      // Coin has already been spend
    ZRSQ_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZRSQ_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    bool CreateZRSQOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    ...
    */
    ...
    bool fBackupMints;
    std::unique_ptr<CzRSQTracker> zpivTracker;
    ...
    void setZWallet(CzRSQWallet* zwallet);
    CzRSQWallet* getZWallet();
    ...
    /// Get 10000 RSQ output and keys which can be used for the Masternode
    ...
    /** zRSQ reset */
    boost::signals2::signal<void()> NotifyzRSQReset;
